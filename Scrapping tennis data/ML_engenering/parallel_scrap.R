...
year_list <- unique(V_MATCH_t %>% filter(Season>=2016) %>% pull(Season))
...
with_progress({
  ...
  p <- progressor(steps = nrow(V_MATCH_t %>% filter(Season>=2016)))
  
  V_MATCH_final <- future_map_dfr(year_list, function(y) {
    
    year_data <- V_MATCH_t %>% filter(Season == y)
