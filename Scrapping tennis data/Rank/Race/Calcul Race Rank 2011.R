...
# print(n=100,V_RACE_RANK %>% filter(Phase=="Qualification") %>% 
#   group_by(Categorie,Round) %>% count())
...
V_RACE_RANK_t=V_RACE_RANK %>%
  mutate(Round=case_when(Phase=="Main Draw" & Round=="F" & Issue=="W"~"Winner",
                         ...
                         #Phase=="Qualification" & Categorie=="Grand Slam" & Round=="Q-R16"~"Q-1R",
                         ...
                         tournament=="Masters Cup Atp" & Round=="SF" & Issue=="L"~"SF",
                          ...
                          Phase=="Qualification" & !Categorie %in% c("ATP 1000") & Round=="Q-1R"~"Q-R16",
...
V_RACE_RANK_t2=V_RACE_RANK_t2 %>% 
  mutate(Categorie=case_when(is.na(Categorie)==TRUE & tournament %in% c("Davis Cup","United Cup","Atp Cup")~"Team Cup",
                             ...
                             TRUE~Categorie)) %>% 
  mutate(Ranking_points=ifelse(is.na(Ranking_points)==T,0,Ranking_points))
...
#calculate_race_points("Sinner Jannik")
...
for (i in year){
  ...
  V_RACE_RANK_p1=V_RACE_RANK_1 %>% 
    ...
    summarise(Race_points=max(Ranking_points,na.rm = T))
...
calculate_race_points("Federer Roger")

save(V_RACE_RANK_F,file = paste0(getwd(),"/Scrapping tennis data/Rank/V_RACE_RANK_2009.RData"))
