...
data2=data %>% select(tourney_date,tourney_name,winner_name,loser_name,round,winner_rank,loser_rank,winner_rank_points,loser_rank_points,winner_hand,loser_hand,winner_ht,loser_ht,winner_id,loser_id,winner_age,loser_age,winner_ioc,loser_ioc)
...
#             left join test c on c.tournament=a.tournament and c.series=a.series")

# Merging entre les 2 bases

merge=sqldf("select distinct a.*,b.PSW,b.PSL,b.AvgW,b.AvgL
      from data2 a 
...
NA_df=merge3 %>% filter(is.na(merge3$AvgW)==TRUE)
