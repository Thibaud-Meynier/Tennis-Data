...
path_9='C:/Users/Thiti/Desktop/Tennis-Data/Bet_data/2009.xlsx'

data_9=read_xlsx(path=path_9)
...
x=unique(data_9$Tournament) %>% data.frame()
...
Round_pivot=data.frame(Round_data2=unique(data_9$Round),
...
data_9$Tournament[data_9$Tournament=="French Open"]="Roland Garros"
data_9$Series[data_9$Location=="Hamburg"]="ATP500"
#data_9$Series[data_9$Location=="Nottingham"]="ATP250"
data_9$Surface[data_9$Location=="Bogota"]="Hard"

data_9=data_9 %>% left_join(Round_pivot %>% select(Round_data2,Round_H),by=c("Round"="Round_data2")) %>% 
...
data_9$WRank=ifelse(data_9$WRank=="N/A",1000,data_9$WRank)
data_9$WPts=ifelse(data_9$WPts=="N/A",10,data_9$WPts)
data_9$LRank=ifelse(data_9$LRank=="N/A",1000,data_9$LRank)
data_9$LPts=ifelse(data_9$LPts=="N/A",10,data_9$LPts)
...
data_9<- mutate(data_9, AvgW = round(rowMeans(select(data_9,c(B365W,EXW,LBW,SJW,UBW)), na.rm = TRUE),2))
data_9<- mutate(data_9, AvgL = round(rowMeans(select(data_9,c(B365L,EXL,LBL,SJL,UBL)), na.rm = TRUE),2))
...
merge=sqldf("select distinct a.*,b.date,b.W1,b.L1,b.W2,b.L2,b.W3,b.L3,b.W4,b.L4,b.W5,b.L5,b.Wsets,b.Lsets,
      ...
      from data2 a 
      left join data_9 b on B.Round_H=a.Round_H and b.WPts=a.winner_rank_points and b.LPts=a.loser_rank_points and b.tourney_name=a.tourney_name")
...
Summary2=data_9 %>% group_by(Tournament) %>% summarise("NB"=sum(is.na(AvgW))) %>% filter(NB>0)
