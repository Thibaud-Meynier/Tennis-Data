...
path_7='C:/Users/Thiti/Desktop/Tennis-Data/Bet_data/2007.xlsx'

data_7=read_xlsx(path=path_7)
...
x=unique(data_7$Tournament) %>% data.frame()
...
patern2=c('Davis Cup','Beijing Olympics','Laver Cup','Atp Cup','United Cup','NextGen Finals')
...
Round_pivot=data.frame(Round_data2=unique(data_7$Round),
                       Round_data=c("RR","QF","SF","F","R128","R64","R32","R16"),
                       Round_H=c("RR","QF","SF","F","R1","R2","R3","R4")) %>% data.frame()
...
when Series='ATP250' and tourney_name not in ('Queens Club','Kitzbuhel') and round='R32' then 'R128' 
when Series='ATP250' and tourney_name not in ('Queens Club','Kitzbuhel') and round='R16' then 'R64'

when Series='ATP250' and tourney_name in ('Queens Club','Kitzbuhel') and round='R64' then 'R128'
when Series='ATP250' and tourney_name in ('Queens Club','Kitzbuhel') and round='R32' then 'R64'
when Series='ATP250' and tourney_name in ('Queens Club','Kitzbuhel') and round='R16' then 'R32'

when Series='ATP500' and tourney_name not in ('Barcelona','Tokyo','New Haven','Washington') and round='R32' then 'R128' 
when Series='ATP500' and tourney_name not in ('Barcelona','Tokyo','New Haven','Washington') and round='R16' then 'R64'

when Series='ATP500' and tourney_name in ('Barcelona','Tokyo','New Haven','Washington') and round='R64' then 'R128'
when Series='ATP500' and tourney_name in ('Barcelona','Tokyo','New Haven','Washington') and round='R32' then 'R64'
when Series='ATP500' and tourney_name in ('Barcelona','Tokyo','New Haven','Washington') and round='R16' then 'R32'
...
data_7$Tournament[data_7$Tournament=="French Open"]="Roland Garros"
#data_7$Series[data_7$Location=="Hamburg"]="ATP500"
#data_7$Series[data_7$Location=="Nottingham"]="ATP250"
data_7$Surface[data_7$Location=="Bogota"]="Hard"
...
data_7$WRank=ifelse(data_7$WRank=="N/A",1000,data_7$WRank)
data_7$WPts=ifelse(data_7$WPts=="N/A",10,data_7$WPts)
data_7$LRank=ifelse(data_7$LRank=="N/A",1000,data_7$LRank)
data_7$LPts=ifelse(data_7$LPts=="N/A",10,data_7$LPts)
...
data_7<- mutate(data_7, AvgW = round(rowMeans(select(data_7,c(B365W,EXW,CBW,PSW,UBW)), na.rm = TRUE),2))
data_7<- mutate(data_7, AvgL = round(rowMeans(select(data_7,c(B365L,EXL,CBL,PSL,UBL)), na.rm = TRUE),2))
...
merge=sqldf("select distinct a.*,b.date,b.W1,b.L1,b.W2,b.L2,b.W3,b.L3,b.W4,b.L4,b.W5,b.L5,b.Wsets,b.Lsets,
      ...
      from data2 a 
      left join data_7 b on B.Round_H=a.Round_H and b.WPts=a.winner_rank_points and b.LPts=a.loser_rank_points and b.tourney_name=a.tourney_name")
...
Summary2=data_7 %>% group_by(Tournament) %>% summarise("NB"=sum(is.na(AvgW))) %>% filter(NB>0)
