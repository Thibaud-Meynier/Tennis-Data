...
path_12='C:/Users/Thiti/Desktop/Tennis-Data/Bet_data/2012.xlsx'

data_12=read_xlsx(path=path_12)
...
x=unique(data_12$Tournament) %>% data.frame()
...
Round_pivot=data.frame(Round_data2=unique(data_12$Round),
...
when Series='ATP500' and tourney_name not in ('Barcelona') and round='R32' then 'R128' 
when Series='ATP500' and tourney_name not in ('Barcelona') and round='R16' then 'R64'

when Series='ATP500' and tourney_name in ('Barcelona') and round='R64' then 'R128'
when Series='ATP500' and tourney_name in ('Barcelona') and round='R32' then 'R64'
when Series='ATP500' and tourney_name in ('Barcelona') and round='R16' then 'R32'
...
data_12$Tournament[data_12$Tournament=="French Open"]="Roland Garros"
data_12$Series[data_12$Location=="Hamburg"]="ATP500"
#data_12$Series[data_12$Location=="Nottingham"]="ATP250"
data_12$Surface[data_12$Location=="Bogota"]="Hard"

data_12=data_12 %>% left_join(Round_pivot %>% select(Round_data2,Round_H),by=c("Round"="Round_data2")) %>% 
...
data_12$WRank=ifelse(data_12$WRank=="N/A",1000,data_12$WRank)
data_12$WPts=ifelse(data_12$WPts=="N/A",10,data_12$WPts)
data_12$LRank=ifelse(data_12$LRank=="N/A",1000,data_12$LRank)
data_12$LPts=ifelse(data_12$LPts=="N/A",10,data_12$LPts)
...
merge=sqldf("select distinct a.*,b.date,b.W1,b.L1,b.W2,b.L2,b.W3,b.L3,b.W4,b.L4,b.W5,b.L5,b.Wsets,b.Lsets,
      ...
      from data2 a 
      left join data_12 b on B.Round_H=a.Round_H and b.WPts=a.winner_rank_points and b.LPts=a.loser_rank_points and b.tourney_name=a.tourney_name")
...
Summary2=data_12 %>% group_by(Tournament) %>% summarise("NB"=sum(is.na(AvgW))) %>% filter(NB>0)

list_na=merge %>% filter(tourney_name %in% unique(Summary$tourney_name) & is.na(Date)==T & tourney_name!='London Olympics')
