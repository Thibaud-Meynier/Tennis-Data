...
path_15='C:/Users/Thiti/Desktop/Tennis-Data/Bet_data/2015.xlsx'

data_15=read_xlsx(path=path_15)
...
x=unique(data_15$Tournament) %>% data.frame()
...
Round_pivot=data.frame(Round_data2=unique(data_15$Round),
...
when Series='ATP250' and tourney_name not in ('Winston-Salem','Nottingham') and round='R32' then 'R128' 
when Series='ATP250' and tourney_name not in ('Winston-Salem','Nottingham') and round='R16' then 'R64'

when Series='ATP250' and tourney_name in ('Winston-Salem','Nottingham') and round='R64' then 'R128'
when Series='ATP250' and tourney_name in ('Winston-Salem','Nottingham') and round='R32' then 'R64'
when Series='ATP250' and tourney_name in ('Winston-Salem','Nottingham') and round='R16' then 'R32'
...
data_15$Tournament[data_15$Tournament=="French Open"]="Roland Garros"
data_15$Series[data_15$Location=="Hamburg"]="ATP500"
data_15$Series[data_15$Location=="Nottingham"]="ATP250"
data_15$Surface[data_15$Location=="Bogota"]="Hard"

data_15=data_15 %>% left_join(Round_pivot %>% select(Round_data2,Round_H),by=c("Round"="Round_data2")) %>% 
...
data_15$WRank=ifelse(data_15$WRank=="N/A",1000,data_15$WRank)
data_15$WPts=ifelse(data_15$WPts=="N/A",10,data_15$WPts)
data_15$LRank=ifelse(data_15$LRank=="N/A",1000,data_15$LRank)
data_15$LPts=ifelse(data_15$LPts=="N/A",10,data_15$LPts)
...
merge=sqldf("select distinct a.*,b.date,b.W1,b.L1,b.W2,b.L2,b.W3,b.L3,b.W4,b.L4,b.W5,b.L5,b.Wsets,b.Lsets,
      ...
      from data2 a 
      left join data_15 b on B.Round_H=a.Round_H and b.WPts=a.winner_rank_points and b.LPts=a.loser_rank_points and b.tourney_name=a.tourney_name")
...
Summary2=data_15 %>% group_by(Tournament) %>% summarise("NB"=sum(is.na(AvgW))) %>% filter(NB>0)
