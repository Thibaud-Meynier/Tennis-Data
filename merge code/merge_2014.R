...
path_14='C:/Users/Thiti/Desktop/Tennis-Data/Bet_data/2014.xlsx'

data_14=read_xlsx(path=path_14)
...
x=unique(data_14$Tournament) %>% data.frame()
...
patern2=c('Davis Cup','Rio Olympics','Laver Cup','Atp Cup','United Cup','NextGen Finals')
...
Round_pivot=data.frame(Round_data2=unique(data_14$Round),
...
when Series='ATP500' and tourney_name not in ('Barcelona','Washington','Hamburg') and round='R32' then 'R128' 
when Series='ATP500' and tourney_name not in ('Barcelona','Washington','Hamburg') and round='R16' then 'R64'

when Series='ATP500' and tourney_name in ('Barcelona','Washington','Hamburg') and round='R64' then 'R128'
when Series='ATP500' and tourney_name in ('Barcelona','Washington','Hamburg') and round='R32' then 'R64'
when Series='ATP500' and tourney_name in ('Barcelona','Washington','Hamburg') and round='R16' then 'R32'
...
data_14$Tournament[data_14$Tournament=="French Open"]="Roland Garros"
data_14$Series[data_14$Location=="Hamburg"]="ATP500"
#data_14$Series[data_14$Location=="Nottingham"]="ATP250"
data_14$Surface[data_14$Location=="Bogota"]="Hard"

data_14=data_14 %>% left_join(Round_pivot %>% select(Round_data2,Round_H),by=c("Round"="Round_data2")) %>% 
...
data_14$WRank=ifelse(data_14$WRank=="N/A",1000,data_14$WRank)
data_14$WPts=ifelse(data_14$WPts=="N/A",10,data_14$WPts)
data_14$LRank=ifelse(data_14$LRank=="N/A",1000,data_14$LRank)
data_14$LPts=ifelse(data_14$LPts=="N/A",10,data_14$LPts)
...
merge=sqldf("select distinct a.*,b.date,b.W1,b.L1,b.W2,b.L2,b.W3,b.L3,b.W4,b.L4,b.W5,b.L5,b.Wsets,b.Lsets,
      ...
      from data2 a 
      left join data_14 b on B.Round_H=a.Round_H and b.WPts=a.winner_rank_points and b.LPts=a.loser_rank_points and b.tourney_name=a.tourney_name")
...
Summary2=data_14 %>% group_by(Tournament) %>% summarise("NB"=sum(is.na(AvgW))) %>% filter(NB>0)

list_na=merge %>% filter(tourney_name %in% unique(Summary$tourney_name) & is.na(Date)==T & tourney_name!='Rio Olympics')
