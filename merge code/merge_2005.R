...
path_5='C:/Users/Thiti/Desktop/Tennis-Data/Bet_data/2005.xlsx'

data_5=read_xlsx(path=path_5)
...
x=unique(data_5$Tournament) %>% data.frame()
...
Round_pivot=data.frame(Round_data2=unique(data_5$Round),
...
data_5$Tournament[data_5$Tournament=="French Open"]="Roland Garros"
data_5$Series[data_5$Location=="Kitzbuhel"]="ATP250"
#data_5$Series[data_5$Location=="Nottingham"]="ATP250"
data_5$Surface[data_5$Location=="Bogota"]="Hard"
data_5$Series=ifelse(data_5$Series=='International','ATP250',
                     ifelse(data_5$Series=='International Gold','ATP500',
                            ifelse(data_5$Series=='Masters','Masters 1000',data_5$Series)))

data_5=data_5 %>% left_join(Round_pivot %>% select(Round_data2,Round_H),by=c("Round"="Round_data2")) %>% 
...
data_5$WRank=ifelse(data_5$WRank=="N/A",1000,data_5$WRank)
data_5$WPts=ifelse(data_5$WPts=="N/A",10,data_5$WPts)
data_5$LRank=ifelse(data_5$LRank=="N/A",1000,data_5$LRank)
data_5$LPts=ifelse(data_5$LPts=="N/A",10,data_5$LPts)
...
data_5<- mutate(data_5, AvgW = round(rowMeans(select(data_5,c(B365W,EXW,CBW,PSW,IWW)), na.rm = TRUE),2))
data_5<- mutate(data_5, AvgL = round(rowMeans(select(data_5,c(B365L,EXL,CBL,PSL,IWL)), na.rm = TRUE),2))
...
merge=sqldf("select distinct a.*,b.date,b.W1,b.L1,b.W2,b.L2,b.W3,b.L3,b.W4,b.L4,b.W5,b.L5,b.Wsets,b.Lsets,
      ...
      from data2 a 
      left join data_5 b on B.Round_H=a.Round_H and b.WRank=a.winner_rank and b.LRank=a.loser_rank and b.tourney_name=a.tourney_name")
...
Summary2=data_5 %>% group_by(Tournament) %>% summarise("NB"=sum(is.na(AvgW))) %>% filter(NB>0)
