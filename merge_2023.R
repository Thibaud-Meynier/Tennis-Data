...
path_23='C:/Users/Thiti/Desktop/Tennis-Data/Bet_data/2023.xlsx'

data_23=read_xlsx(path=path_23)
...
x=unique(data_23$Tournament) %>% data.frame()
...
patern2=c('Davis Cup','Olympics','Laver Cup','Atp Cup','United Cup','NextGen Finals')
...
z$tourney_name2[z$tourney_name=="Rio De Janeiro"]="Rio de Janeiro"
z$tourney_name2[z$tourney_name=="Astana"]="Nur-Sultan"
...
Round_pivot=data.frame(Round_data2=unique(data_23$Round),
...
when Series='Masters 1000' and tourney_name not in ('Miami Masters','Indian Wells Masters','Shanghai Masters','Rome Masters','Madrid Masters') and round='R64' then 'R128'
when Series='Masters 1000' and tourney_name not in ('Miami Masters','Indian Wells Masters','Shanghai Masters','Rome Masters','Madrid Masters') and round='R32' then 'R64'
when Series='Masters 1000' and tourney_name not in ('Miami Masters','Indian Wells Masters','Shanghai Masters','Rome Masters','Madrid Masters') and round='R16' then 'R32'
...
data_23$Tournament[data_23$Tournament=="French Open"]="Roland Garros"
data_23$Series[data_23$Location=="Hamburg"]="ATP500"

data_23=data_23 %>% left_join(Round_pivot %>% select(Round_data2,Round_H),by=c("Round"="Round_data2")) %>% 
...
data_23$WRank=ifelse(data_23$WRank=="N/A",1000,data_23$WRank)
data_23$WPts=ifelse(data_23$WPts=="N/A",10,data_23$WPts)
data_23$LRank=ifelse(data_23$LRank=="N/A",1000,data_23$LRank)
data_23$LPts=ifelse(data_23$LPts=="N/A",10,data_23$LPts)
...
# Redressement individuel car présence de données éronnées entre les 2 bases 

data2$loser_rank_points[data2$loser_name=="Guido Pella"&data2$tourney_name=="Australian Open" & data2$round=="R128"]=318
data2$loser_rank[data2$loser_name=="Guido Pella"&data2$tourney_name=="Australian Open" & data2$round=="R128"]=181

data2$winner_rank_points[data2$winner_name=="Kevin Anderson"&data2$tourney_name=="Newport"]=221
data2$winner_rank[data2$winner_name=="Kevin Anderson"&data2$tourney_name=="Newport"]=296

data2$loser_rank_points[data2$loser_name=="Kevin Anderson"&data2$tourney_name=="Newport"]=221
data2$loser_rank[data2$loser_name=="Kevin Anderson"&data2$tourney_name=="Newport"]=296

data2$loser_rank_points[data2$loser_name=="Eduardo Nava"&data2$tourney_name=="Roland Garros"]=238
data2$loser_rank[data2$loser_name=="Eduardo Nava"&data2$tourney_name=="Roland Garros"]=242

data2$loser_rank_points[data2$loser_name=="Stefanos Tsitsipas"&data2$tourney_name=="Tour Finals"]=4435
data2$loser_rank_points[data2$loser_name=="Andrey Rublev"&data2$tourney_name=="Tour Finals"]=5205
data2$loser_rank_points[data2$loser_name=="Novak Djokovic"&data2$tourney_name=="Tour Finals"]=11445
data2$winner_rank_points[data2$winner_name=="Novak Djokovic"&data2$tourney_name=="Tour Finals"]=11445
...
merge=sqldf("select distinct a.*,b.date,b.W1,b.L1,b.W2,b.L2,b.W3,b.L3,b.W4,b.L4,b.W5,b.L5,b.Wsets,b.Lsets,
      ...
      from data2 a 
      left join data_23 b on B.Round_H=a.Round_H and b.WPts=a.winner_rank_points and b.LPts=a.loser_rank_points and b.tourney_name=a.tourney_name")

# Test de correspondance de la jointure 

NA_df=merge %>% filter(is.na(merge$AvgW)==TRUE)

Summary=NA_df %>% group_by(tourney_name) %>% summarise("NB"=n()) %>% filter(str_starts(tourney_name,"Davis Cup|United Cup|Laver Cup|NextGen Finals")==FALSE)

Summary2=data_23 %>% group_by(Tournament) %>% summarise("NB"=sum(is.na(AvgW))) %>% filter(NB>0)

# Pella et Chardy AO 181 R 318 pts NA
# Anderson Newport +  Spizzirri 296 R 221 Pts Ander NA spirrizi
# Tour Finals Djokovic 11445 + Tsitsipas 4435 + Rublev 5205
# Dhamne Pune NA
# Nava RG 242 R 238 pts
# Intervertion Shanghai fait
# Balazs US Open NA 
# Raonic Hertogenbosh NA dans les 2
# MArrakech intervertion fait 
