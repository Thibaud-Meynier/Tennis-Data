...
x=unique(data_19$Tournament) %>% data.frame()
...
Round_pivot=data.frame(Round_data2=unique(data_19$Round),
...
data_19$Tournament[data_19$Tournament=="French Open"]="Roland Garros"
data_19$Series[data_19$Location=="Hamburg"]="ATP500"

data_19=data_19 %>% left_join(Round_pivot %>% select(Round_data2,Round_H),by=c("Round"="Round_data2")) %>% 
...
data_19$WRank=ifelse(data_19$WRank=="N/A",1000,data_19$WRank)
data_19$WPts=ifelse(data_19$WPts=="N/A",10,data_19$WPts)
data_19$LRank=ifelse(data_19$LRank=="N/A",1000,data_19$LRank)
data_19$LPts=ifelse(data_19$LPts=="N/A",10,data_19$LPts)
...
data2$loser_rank_points[data2$loser_name=="Ze Zhang"&data2$tourney_name=="Shanghai Masters"]=278
data2$loser_rank[data2$loser_name=="Ze Zhang"&data2$tourney_name=="Shanghai Masters"]=179
...
merge=sqldf("select distinct a.*,b.date,b.W1,b.L1,b.W2,b.L2,b.W3,b.L3,b.W4,b.L4,b.W5,b.L5,b.Wsets,b.Lsets,
      ...
      from data2 a 
      left join data_19 b on B.Round_H=a.Round_H and b.WPts=a.winner_rank_points and b.LPts=a.loser_rank_points and b.tourney_name=a.tourney_name")
...
Summary2=data_19 %>% group_by(Tournament) %>% summarise("NB"=sum(is.na(AvgW))) %>% filter(NB>0)

list_na=merge %>% filter(tourney_name %in% c('Shanghai Masters','Winston-Salem') & is.na(Date)==T)
