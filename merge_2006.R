...
path_6='C:/Users/Thiti/Desktop/Tennis-Data/Bet_data/2006.xlsx'

data_6=read_xlsx(path=path_6)
...
x=unique(data_6$Tournament) %>% data.frame()
...
Round_pivot=data.frame(Round_data2=unique(data_6$Round),
...
when Series='ATP500' and tourney_name not in ('Barcelona','Tokyo','New Haven','Washington','Stuttgart') and round='R32' then 'R128' 
when Series='ATP500' and tourney_name not in ('Barcelona','Tokyo','New Haven','Washington','Stuttgart') and round='R16' then 'R64'

when Series='ATP500' and tourney_name in ('Barcelona','Tokyo','New Haven','Washington','Stuttgart') and round='R64' then 'R128'
when Series='ATP500' and tourney_name in ('Barcelona','Tokyo','New Haven','Washington','Stuttgart') and round='R32' then 'R64'
when Series='ATP500' and tourney_name in ('Barcelona','Tokyo','New Haven','Washington','Stuttgart') and round='R16' then 'R32'
...
data_6$Tournament[data_6$Tournament=="French Open"]="Roland Garros"
data_6$Series[data_6$Location=="Kitzbuhel"]="ATP250"
#data_6$Series[data_6$Location=="Nottingham"]="ATP250"
data_6$Surface[data_6$Location=="Bogota"]="Hard"
data_6$Series=ifelse(data_6$Series=='International','ATP250',
                     ifelse(data_6$Series=='International Gold','ATP500',
                            ifelse(data_6$Series=='Masters','Masters 1000',data_6$Series)))

data_6=data_6 %>% left_join(Round_pivot %>% select(Round_data2,Round_H),by=c("Round"="Round_data2")) %>% 
...
data_6$WRank=ifelse(data_6$WRank=="N/A",1000,data_6$WRank)
data_6$WPts=ifelse(data_6$WPts=="N/A",10,data_6$WPts)
data_6$LRank=ifelse(data_6$LRank=="N/A",1000,data_6$LRank)
data_6$LPts=ifelse(data_6$LPts=="N/A",10,data_6$LPts)
...
data_6<- mutate(data_6, AvgW = round(rowMeans(select(data_6,c(B365W,EXW,CBW,PSW,UBW)), na.rm = TRUE),2))
data_6<- mutate(data_6, AvgL = round(rowMeans(select(data_6,c(B365L,EXL,CBL,PSL,UBL)), na.rm = TRUE),2))
...
merge=sqldf("select distinct a.*,b.date,b.W1,b.L1,b.W2,b.L2,b.W3,b.L3,b.W4,b.L4,b.W5,b.L5,b.Wsets,b.Lsets,
      ...
      from data2 a 
      left join data_6 b on B.Round_H=a.Round_H and b.WPts=a.winner_rank_points and b.LPts=a.loser_rank_points and b.tourney_name=a.tourney_name")
...
Summary2=data_6 %>% group_by(Tournament) %>% summarise("NB"=sum(is.na(AvgW))) %>% filter(NB>0)
